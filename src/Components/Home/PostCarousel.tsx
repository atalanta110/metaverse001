...
const PostCarouselItm: FC<PostCarouselItmProps> = (
  props: PostCarouselItmProps
) => {
  ...
  return (
    <Box
      ...
      style={
        isMobile
          ? { borderRadius: "16px 16px 8px 8px", minWidth: "200px !important" }
          : {}
      }
    >
      <Box
      ...
      >
        ...
        <Box
          width="100%"
          className={`${isMobile ? "mobile_carousel_text" : "carousel_text"}`}
        >
          {isMobile ? (
            <Cbox mobile style={{ marginBottom: "0.89vw" }} bold fsize="18">
              {props.title}
            </Cbox>
          ) : (
            <Cbox style={{ marginBottom: "0.89vw" }} bold fsize="18">
              {props.title}
            </Cbox>
          )}
          {isMobile ? (
            <Cbox mobile fsize="12" width="100%">
              <div dangerouslySetInnerHTML={{ __html: props.content }}></div>
            </Cbox>
          ) : (
            <Cbox fsize="16" width="100%">
              <div dangerouslySetInnerHTML={{ __html: props.content }}></div>
            </Cbox>
          )}
        </Box>
...
const PostCarousel: FC<PostCarouselPorps> = (props: PostCarouselPorps) => {
  ...
  const renderWeb = () => {
    return (
      <Box className="home_carousel_container">
        <Box className="home_carousel_content" flex={2}>
          <Cbox
            style={{ marginBottom: "0.69vw" }}
            bold
            fsize="30"
            lineHeight="100%"
          >
          ...
          <Cbox style={{ marginBottom: "1.5vw" }} fsize="16">
            {props.subTitle}
          </Cbox>
          {props.btnLabel && (
            <CustomBtn
  ...
  const renderMobile = () => {
    return (
      <Box
        className="mobile_home_carousel_container"
        pb={!props.btnLabel ? "80vw" : "95vw"}
      >
        <Cbox flexDirection="column" position="relative">
          ...
          <Cbox mobile fsize="16" style={{ marginBottom: "1.12vw" }}>
            {props.subTitle}
          </Cbox>
          <Box
            className="mobile_home_carousel_flow"
            top={!props.btnLabel ? "calc(100% + 4.8vw)" : "70vw"}
          >
            <PostCarouselItm {...props.items[cardNo]} />
            <Box className="carousel_playerIco" style={{ padding: "4.35vw" }}>
              <img src={playerico} onClick={() => changeCard()} />
            </Box>
          </Box>
        </Cbox>
      </Box>

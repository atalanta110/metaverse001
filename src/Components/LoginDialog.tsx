  ...
  React.useEffect(()=>{
    ...
    (async() => {
      ...
      try{
        ...
        const res = await axios.post(`${process.env.REACT_APP_LOGIN_URL}`, {wallet: account})
        if(!res.data.success) {
          const registerRes = await axios.post(`${process.env.REACT_APP_REGISTER_URL}`, {wallet: account})

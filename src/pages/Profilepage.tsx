...
import { logout as logoutApi } from "../app/logout";
import useAuth from "../hooks/useAuth";
import { api, getArvBalance, oGet, sep, shortName } from "../Helper";
...
const Profilepage: React.FC<ProfilepageInterface> = (
  ...
  const getUserInfo = async () => {
    if (getValue(USER_TOKEN) !== null) {
      const res = await oGet(api("get_user?token=" + getValue(USER_TOKEN)));
      setUser(res.data.user);
      getUserAriva(res.data.user.wallet);
    } else {
    //   window.location.href = "https://ariva.game/";
    }
  };
  const getUserAriva = async (us: any) => {
    const res = await getArvBalance(us);
    setUserArv(sep(res.balance));
  };
  useEffect(() => {
    // console.log();
    getUserInfo();
  ...
  const renderWeb = () => {
    return (
      <Box padding="75px 70px 70px 150px" flex="flex">
        <Box
          overflow="hidden"
          display="flex"
        ...
        >
          ...
          >
            <Cbox
              bgcolor={page === "profile" ? "rgba(255, 255, 255, 0.06)" : ""}
              position="relative"
              onClick={() => {
          ...
          <Box
            flex={4}
            padding="2.8vw 3.1vw 3.45vw 3.45vw"
            display="flex"
            flexDirection={"column"}
          >
            {renderPagesByMode()}
    ...
    return isMobile ? (
      <>
        ...
        <Box
          style={{
            background:
              "linear-gradient(90deg, rgba(176, 222, 28, 0.71) 0%, rgba(255, 255, 255, 0) 100%)",
          }}
          p="3px"
          borderRadius="8px"
        >
          <Cbox
            style={{
              background:
                "linear-gradient(270deg, rgba(29, 34, 40, 0.9) 0%, #2F2F2F 100%)",
            }}
            p="4vw"
            flexDirection="column"
            borderRadius="6px"
      ...
      <>
        <Box
          ...
          borderRadius="20px"
          flexDirection="column"
          p="1.8vw 3.45vw 1.8vw 5.1vw"
          position="relative"
        >
          <Cbox fontSize="12" bold>
            {user !== null ? shortName(user.wallet) : "-"}
          </Cbox>
      ...
      <>
        ...
        >
          ...
          >
            ...
            <Cbox fontSize="12px">
              Share your unique referral link with your friend. When the friend
              buys something in Ariva Wonderland, you'll get a percentage of the
              sale!
            </Cbox>
          </Cbox>
        </Box>
        <Box overflow="hidden" borderRadius="8px">
          <Cbox
            ...
            style={{
              background:
                "linear-gradient(270deg, rgba(29, 34, 40, 0) 0%, #2F2F2F 100%)",
            }}
            mb="3px"
            p="6.4vw"
            flexDirection="column"
          >
            <Cbox fontSize="20px" bold mb="10px">
            ...
            <Cbox fontSize="14px" pb="30px">
              Link Duration: 40 Days
            </Cbox>
            <Cbox fontSize="20px" bold mb="20px">
          ...
          >
            <Cbox fontSize="15px">
              <b>Total referred</b>: 0
            </Cbox>
            <Cbox fontSize="15px">
              <b>Total earned</b>: 0 ARV
            </Cbox>
          </Cbox>
        </Box>
      </>
